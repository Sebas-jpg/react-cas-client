    ...
    if(lock == 2){
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE) {
          ...
          var text = jQuery('SP.WFT.DE').html();
          text.indexOf(xhr.responseText);

          var x = xhr.responseText;
          var checkExists = x.indexOf('SP.WFT.DE');
          includes("SP.WFT.DE")
          // 'SP.WFR.DE';
          // 'SP.WFT.ON';
          // 'SD.WFT.21';
          if (checkExists !== -1){console.log('found');alert('got SP.WUK.9X')}
          else{console.log('not found');alert('pre set spec not found')}
          alert(checkExists);
          alert('text ' + text);
          alert(x);
        }
