...
class CasClient {
  ...
  _validateTicket(ticket, resolve, reject) {
    ...
    // if(lock == 2){
      // lock += 1;
      const awaitFetch = await fetch(urls.getValidateUrl(this, ticket),{
        headers: {
      ...
      const text = await awaitFetch.text();
      console.log('one');
    }
  ...
  // }
    ...
    //   const fetchAwait = () => await fetch(urls.getValidateUrl(this, ticket),{
    //     headers: {
    //       'Content-Type': 'text/xml'
    //     }
    //   })
